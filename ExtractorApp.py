import os
from tkinter import Tk, scrolledtext, Button, END
import subprocess
...
class GUI:
    ...
    def extract_wifi_passwords(self):
        ...
        data = (subprocess.run(['netsh', 'wlan', 'show', 'profiles'], shell=True, capture_output=True, text=True)).stdout.split('\n')
        ...
        for i in profiles:
            results = (subprocess.run(['netsh', 'wlan', 'show', 'profile', i, 'key=clear'], shell=True, capture_output=True, text=True)).stdout.split('\n')
